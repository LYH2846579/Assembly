...
data segment
    ...
    lfLen  db ?                    ;需要填充的空格的长度
    stbuf  db 255,0,512 DUP(0)     ;初始数据存储的地方                          ;仍然无法解决缓冲区溢出的问题!
    delbuf db 255,0,512 DUP(0)     ;处理之后的数据存储的地方                    
    USD    db '$'
     ...
     ;addpoint:  add word ptr ds:[25],1          ;自增1
      ...
      special:  mov cx,ds:[25]       ;将有效长度赋值给cx ->以字节为单位         ;跳转之前这个不能忘
        ...
        add1:   mov ds:[bp],' '                             ;是不是得考虑上来就是.的情况????????
                inc bp
                loop add1
                ;接下来将字符串复制                                                                             ;在整个对于标点符号的处理过程之中，
                mov cx,ds:[25]       ;将有效长度赋值给cx ->以字节为单位                                         ;均使用bx寄存器对其进行操作，ax寄存器
                                                                                                              ;存储的填充状态保持不变!
                ;此循环将原文复制到目标段                                                                       ;否则会出现填充的短语句对齐异常问题!
        copy:   mov bl,ds:[di]
        ...
        tag2:   cmp byte ptr ds:[si-1],'.'                              ;判断是否为省略号
                ...
                je  tag3
                mov bl,ds:[si]                          ;tag2修复了针对于省略号的不完全显示问题
                mov ds:[bp],bl
                inc bp

        tag3:   cmp byte ptr ds:[si+1],'$'                                                        ;判断是否为 最后一个符号
                jne tag4                                        ;该项存在!?重复添加的问题!
                cmp byte ptr ds:[si],'.'                                ;附加判断是否为.
                jne tag4                                        ;这样一来就会只在...的时候将该符号加上
                mov bl,ds:[si]
                mov ds:[bp],bl 
                inc bp  
...
;                mov ds:[bp],al                          ;将 '?'加入
        ...
        flag3:  inc bp
                ...
                add dx,2                        ;成功修复超短序列位于首句的时候的异常问题!                      ->1,2,3, uydj vcjqwfijwvmdvc wwkj gf.
...
;修复的问题:
;       1.特殊标点符号的显示问题，及显示位置问题        -> [!,?]
;       2.省略号的特殊处理问题                         -> [...]
;       3.末尾符号的显示问题
;       4.超短句的对齐问题
;       5.首句的对齐问题
;       6.缓冲区溢出问题
;       7.异常处理问题                                 -> 目前仅仅可以处理全.....的序列
;       8.定义的数据段起始位置问题                      mov dx,delstr  add dx,2
;
;
;
;
